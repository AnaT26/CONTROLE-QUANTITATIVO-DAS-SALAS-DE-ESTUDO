...
def cadastrar(nome, dre, curso, email, senha, repetesenha):
    ...
    if e:
        ...
        if senha == repetesenha:
            conn = sqlite3.connect(os.path.join(basedir, 'te.db'))
            c = conn.cursor()
            c.executescript("INSERT INTO pessoa(dre, nome, curso, senha, email) VALUES('"+dre+"','"+nome+"','"+curso+"','"+senha+"','"+email+"')")
            c.executescript("INSERT INTO user(email, senha) VALUES('"+email+"','"+senha+"')")
            conn.commit()
